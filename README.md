...
```sh
make docker-build docker-push IMG=<some-registry>/guarduim:tag
```
...
```sh
make deploy IMG=<some-registry>/guarduim:tag
```
...
```sh
make build-installer IMG=<some-registry>/guarduim:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/guarduim/<tag or branch>/dist/install.yaml
```
